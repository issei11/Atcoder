...
class Solver:
    ...
    def _move(self, lim=None):
        ...
        def can_Connect(i, j, v, V):
            i3 = i + v[0]
            j3 = j + v[1]
            while 0 < i3 < self.N and 0 < j3 < self.N:
                if self.C[i3][j3] == 0:
                    i3 += v[0]
                    j3 += v[1]
                    continue
                elif self.C[i3][j3] == self.C[i-V[0]][j-V[1]]:
                    return True
                return False
            return False

        if lim is None:
            lim = self.K * (self.K-1)*10
        moves = []
        cnt = 0
        for i in range(self.N):
            for j in range(self.N):
                ...
                if cnt > lim:
                    break
                can = 0
                if can_Connect(i2,j2,self.VECS[0],v) == False:
                    can += 1
                if can_Connect(i2,j2,self.VECS[1],v) == False:
                    can += 1
                if can_Connect(i2,j2,self.VECS[2],v) == False:
                    can += 1
                if can_Connect(i2,j2,self.VECS[3],v) == False:
                    can += 1
                if can == 4:
                    continue
